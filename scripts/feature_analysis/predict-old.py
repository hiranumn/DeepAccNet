...
def main():
    ...
    # Load the best model
    net = dan.DeepAccNet(num_chunks   = 5,
                        num_channel  = 128,
                        twobody_size = 49 if bert else 33)

    checkpoint = torch.load(join(name, "best.pkl"))
    ...
    dirpath = join(distination, basename(normpath(name)))
    ...
    for pname in proteins:
        print(pname)
        dirpath = join(distination, basename(normpath(name)), pname)
        if not isdir(dirpath):
            os.mkdir(dirpath)
        ...
        if pname in decoys.proteins:
            with torch.no_grad():
                for i in range(len(decoys.samples_dict[pname])):
                    ...
                    filepath = join(distination, basename(normpath(name)), pname, samplename+".npz")
